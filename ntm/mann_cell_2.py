...
class MANNCell():
    def __init__(self, rnn_size, memory_size, memory_vector_dim, head_num, gamma=0.95, k_strategy='separate'):
        ...
        self.controller = tf.contrib.rnn.BasicLSTMCell(self.rnn_size)
        self.step = 0
        self.gamma = gamma
        self.k_strategy = k_strategy

        self.W_key = tf.Variable(tf.random_uniform([self.head_num,
                                                    self.controller.output_size,
                                                    self.memory_vector_dim], minval=-0.1, maxval=0.1))
        self.b_key = tf.Variable(
            tf.random_uniform([self.head_num, self.memory_vector_dim], minval=-0.1, maxval=0.1)
        )
        self.W_add = tf.Variable(
            tf.random_uniform([self.head_num, self.controller.output_size, self.memory_vector_dim], minval=-0.1,
                              maxval=0.1)
        )
    ...
    def __call__(self, x, prev_state):
        ...
        wlu_tm1 = tf.reduce_sum(tf.one_hot(wlu_tm1_[:, -self.head_num:], depth=self.memory_size),
                                axis=1)  # (batches, head_num, memory_size) -> (batches, memory_size)
        ww_t = sigma_t * wr_tm1 + (1. - sigma_t) * tf.expand_dims(wlu_tm1,
                                                                  axis=1)  # (batches, 1 -> head_num, memory_size)
        ...
        norm_product = tf.sqrt(tf.reduce_sum(tf.square(k_t), axis=2, keep_dims=True)) * \
                       tf.expand_dims(tf.sqrt(tf.reduce_sum(tf.square(M_t), axis=2)),
                                      axis=1)  # (batches, heads, memory_size)
